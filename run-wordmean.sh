#!/bin/bash
# test hadoop wordmean: calculate average word length
...
# 执行 WordMean
echo -e "\n running wordmean"
# 修正后命令（通过HADOOP_OPTS传递log4j配置）
OUTPUT=$(hadoop jar $HADOOP_HOME/share/hadoop/mapreduce/hadoop-mapreduce-examples-2.7.2.jar wordmean "$1" "$2" 2>&1)
EXIT_CODE=$?
...
# 记录输出
echo "wordmean output:" >> $LOG_FILE
hdfs dfs -cat $2/part* >> $LOG_FILE 2>&1
echo "" >> $LOG_FILE

# 单独提取最后一行 "The an is: ..." 并明确标记（可选）
MEDIAN_LINE=$(echo "$OUTPUT" | tail -1 | grep "^The mean is:")
